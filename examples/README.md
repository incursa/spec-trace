...
Canonical example artifacts are the `.cue` files in each example directory. The `.md` siblings are generated from those sources for readability.
...
The generated `*.evidence.json` files in the examples are valid inputs for
[`../scripts/Validate-SpecTraceEvidence.ps1`](../scripts/Validate-SpecTraceEvidence.ps1)
and can be folded into the HTML and JSON report generated by
[`../scripts/Render-SpecTraceAttestation.ps1`](../scripts/Render-SpecTraceAttestation.ps1).

The example set is also used as a validation target for [`scripts/Test-SpecTraceRepository.ps1`](../scripts/Test-SpecTraceRepository.ps1), including duplicate IDs, unresolved direct links, wrong target kinds, reciprocal trace consistency, namespace alignment, and profile checks.

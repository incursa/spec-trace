...
The reference repository is JSON-first. Canonical artifacts are authored in `*.json`, and JSON Schema validates their document shape.

## Core Vocabulary

### Specification

A specification is a canonical artifact that groups one or more related requirements for a capability, behavior area, interface, or narrow technical concern.
...
The canonical authored form is a JSON record:
...
Inline identifier references in prose are useful navigation aids, but they are not typed trace edges by themselves.

## Static Trace Versus Derived Reporting

The authored trace graph lives in canonical JSON artifacts.
...
The repository validates canonical JSON inputs through a deterministic local toolchain:

- JSON Schema checks structure, required fields, enums, and identifier patterns.
- The repository validator builds a catalog of artifact IDs and nested requirement IDs.
- Validation fails on duplicates, broken references, namespace drift, reciprocal link gaps, and profile-specific issues.

## Reading The Repo

Use this order:

1. canonical JSON artifacts under [`specs/requirements/spec-trace/`](./specs/requirements/spec-trace/)
2. root guidance such as [`authoring.md`](./authoring.md) and [`layout.md`](./layout.md)
3. worked examples under [`examples/`](./examples/)
4. generated outputs and explainers when you need reporting context
